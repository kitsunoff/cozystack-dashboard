...
export function KafkaForm({ plural, namespace, apiGroup, apiVersion, kind, backHref, openAPISchema }: CustomFormProps) {
  const schema = openAPISchema ?? {};
  const kafkaSchema = schemaAt(schema, ["kafka"]);
  const zkSchema = schemaAt(schema, ["zookeeper"]);

  return (
    <WizardShell schema={schema} plural={plural} namespace={namespace} apiGroup={apiGroup} apiVersion={apiVersion} kind={kind} backHref={backHref} submitLabel="Kafka">
      <Separator />
